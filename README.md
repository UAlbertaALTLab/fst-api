...
- `FST_API_GENERATOR_NORM_FST`: (required) The filename of the generator FST (with extension, without path). If missing, it defaults to "generator-gt-norm.hfstol"
- `FST_API_ANALYSER_NORM_FST`: (required) The filename of the normative analyser FST (with extension, without path). If missing, it defaults to "analyser-gt-norm.hfstol"
- `FST_API_ANALYSER_DESC_FST`: (required) The filename of the descriptive analyser FST (with extension, without path). If missing, it defaults to "analyser-gt-desc.hfstol"
